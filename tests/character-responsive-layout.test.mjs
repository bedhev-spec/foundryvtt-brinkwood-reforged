...
test("Character sheet supports a 700px maximum and 480px responsive minimum", async () => {
  ...
  assert.match(styles, /@container \(max-width: 640px\)[\s\S]*?\.name-alias\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0, 1fr\)[\s\S]*?\.sheet-identity__portrait\s*\{[\s\S]*?grid-row:\s*1;[\s\S]*?\.name-alias > \.sheet-identity__details\s*\{[\s\S]*?grid-row:\s*2;[\s\S]*?\.name-alias > \.sheet-identity__trackers\s*\{[\s\S]*?grid-row:\s*3;/);
  assert.match(styles, /@container \(max-width: 640px\)[\s\S]*?\.name-alias > \.sheet-identity__trackers\s*\{[\s\S]*?margin-top:\s*10px;[\s\S]*?padding-bottom:\s*10px;/);
