...
3. In that release pull request:
   - choose the next immutable version, such as `1.0.0-rc.3`;
   - update `system.json` `version` and its `download` URL to the matching `v1.0.0-rc.3` tag;
...
6. Tag the exact merge commit on `main` with an annotated tag and push it:
   ...
   git tag -a v1.0.0-rc.3 -m "Brinkwood Reforged v1.0.0-rc.3"
